...
## 读取测试集
test = pd.read_csv('data/wechat_algo_data1/test_a.csv', nrows=10000)
test['date_'] = max_day
print(test.shape)
...
trn_x = reduce_mem(trn_x, [f for f in trn_x.columns if f not in ['date_'] + play_cols + y_list])
val_x = reduce_mem(val_x, [f for f in val_x.columns if f not in ['date_'] + play_cols + y_list])   
del train
print('check point after trn_x, val_x')
mem_info()
##################### 线下验证 #####################
'''
uauc_list = []
