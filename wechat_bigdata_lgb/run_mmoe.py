...
if __name__ == "__main__":
    ...
    # feed_embedding = np.random.randn(data['feedid'].max() + 1, 512)
    pretrained_feed_embed_initializer = tf.initializers.identity(feed_embedding)
    ...
    fixlen_feature_columns = [SparseFeat('feedid', vocabulary_size=data['feedid'].max() + 1, embedding_dim=512,
                                         embeddings_initializer=pretrained_feed_embed_initializer)] + [
