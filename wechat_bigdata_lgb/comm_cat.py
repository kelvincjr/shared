...
USER_ACTION_WITH_INFO = os.path.join(DATASET_PATH, "user_action_with_info.csv")
...
def statis_feature(start_day=1, before_day=7, agg='sum'):
    ...
    """
    id_column_list = ["userid", "date_", "feedid", 'authorid', 'videoplayseconds', 'bgm_song_id', 'bgm_singer_id']
    history_data = pd.read_csv(USER_ACTION_WITH_INFO)[id_column_list + FEA_COLUMN_LIST]
    feature_dir = os.path.join(ROOT_PATH, "feature")
    for dim in [["userid"], ["feedid"], ["authorid"], ["bgm_song_id"], ["bgm_singer_id"],
        ...
        for start in range(start_day, END_DAY - before_day + 1):
            ...
            temp = temp.groupby(by=dim).agg([agg]).reset_index()
            temp.columns = list(map(''.join, temp.columns.values))
            temp["date_"] = start + before_day
            res_arr.append(temp)
        dim_feature = pd.concat(res_arr)
        dim_name = "_".join(dim)
        feature_path = os.path.join(feature_dir, dim_name + "_feature.csv")
...
def concat_sample(sample_arr, stage="offline_train"):
    ...
    # 基于 authorid 统计的历史行为的次数
    author_date_feature_path = os.path.join(ROOT_PATH, "feature", "authorid_feature.csv")
    author_date_feature = pd.read_csv(author_date_feature_path)
    author_date_feature = author_date_feature.set_index(["authorid", "date_"])
    # 基于 bgm_song_id 统计的历史行为的次数
    song_date_feature_path = os.path.join(ROOT_PATH, "feature", "bgm_song_id_feature.csv")
    song_date_feature = pd.read_csv(song_date_feature_path)
    song_date_feature = song_date_feature.set_index(["bgm_song_id", "date_"])
    # 基于 bgm_singer_id 统计的历史行为的次数
    singer_date_feature_path = os.path.join(ROOT_PATH, "feature", "bgm_singer_id_feature.csv")
    singer_date_feature = pd.read_csv(singer_date_feature_path)
    singer_date_feature = singer_date_feature.set_index(["bgm_singer_id", "date_"])
    # 基于 userid_authorid 统计的历史行为的次数
    user_author_date_feature_path = os.path.join(ROOT_PATH, "feature", "userid_authorid_feature.csv")
    user_author_date_feature = pd.read_csv(user_author_date_feature_path)
    user_author_date_feature = user_author_date_feature.set_index(["userid", "authorid", "date_"])
    # 基于 userid_bgm_song_id 统计的历史行为的次数
    user_song_date_feature_path = os.path.join(ROOT_PATH, "feature", "userid_bgm_song_id_feature.csv")
    user_song_date_feature = pd.read_csv(user_song_date_feature_path)
    user_song_date_feature = user_song_date_feature.set_index(["userid", "bgm_song_id", "date_"])
    # 基于 userid_bgm_singer_id 统计的历史行为的次数
    user_singer_date_feature_path = os.path.join(ROOT_PATH, "feature", "userid_bgm_singer_id_feature.csv")
    user_singer_date_feature = pd.read_csv(user_singer_date_feature_path)
    user_singer_date_feature = user_singer_date_feature.set_index(["userid", "bgm_singer_id", "date_"])
    for index, sample in enumerate(sample_arr):
        ...
        sample = sample.join(author_date_feature, on=["authorid", "date_"], how="left", rsuffix="_author")
        sample = sample.join(song_date_feature, on=["bgm_song_id", "date_"], how="left", rsuffix="_song")
        sample = sample.join(song_date_feature, on=["bgm_singer_id", "date_"], how="left", rsuffix="_singer")
        sample = sample.join(user_author_date_feature, on=["userid", "authorid", "date_"],
                             how="left", rsuffix="_user_author")
        sample = sample.join(user_song_date_feature, on=["userid", "bgm_song_id", "date_"],
                             how="left", rsuffix="_user_song")
        sample = sample.join(user_singer_date_feature, on=["userid", "bgm_singer_id", "date_"],
                             how="left", rsuffix="_user_singer")
        feed_feature_col = [b + "sum" for b in FEA_COLUMN_LIST]
        user_feature_col = [b + "sum_user" for b in FEA_COLUMN_LIST]
        author_feature_col = [b + "sum_author" for b in FEA_COLUMN_LIST]
        song_feature_col = [b + "sum_song" for b in FEA_COLUMN_LIST]
        singer_feature_col = [b + "sum_singer" for b in FEA_COLUMN_LIST]
        user_author_feature_col = [b + "sum_user_author" for b in FEA_COLUMN_LIST]
        user_song_feature_col = [b + "sum_user_song" for b in FEA_COLUMN_LIST]
        user_singer_feature_col = [b + "sum_user_singer" for b in FEA_COLUMN_LIST]
        sum_features = []
        sum_features += feed_feature_col
        sum_features += user_feature_col
        sum_features += author_feature_col
        sum_features += song_feature_col
        sum_features += singer_feature_col
        sum_features += user_author_feature_col
        sum_features += user_song_feature_col
        sum_features += user_singer_feature_col
        sample[sum_features] = sample[sum_features].fillna(0.0)
        sample[sum_features] = np.log(sample[sum_features] + 1.0)
