...
# from model_double2 import DMBERT
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, test=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        label_list = processor.get_labels() #获得所有事件类型
        if evaluate:
        ...
        features = convert_examples_to_features(   #401985条
            examples,
            label_list,
            args.max_seq_length,
            tokenizer,
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result = evaluate(args, model, tokenizer, prefix=prefix, test=False) #ToDel
