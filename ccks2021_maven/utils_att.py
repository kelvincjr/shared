...
class MAVENProcessor(DataProcessor):
    ...
    def get_test_examples(self, data_dir):
        ...
        logger.info("LOOKING AT {} test".format(data_dir))
        # 在测试集合上运行
        # return self._create_examples(open(os.path.join(data_dir,'test_ori.jsonl'),"r"), "test")
            ...
            if(set_type != "test"):
                for event in data['events']:
                    if event['type']=='None of the above':
                        print("?????????")
                    for mention in event['mention']:   # {'trigger_word': 'observed', 'sent_id': 5, 'offset': [12, 13], 'id': '7fcf445a679aa13511278d321a908bd2'}
                        e_id = "%s-%s" % (set_type, mention['id'])  #'train-7fcf445a679aa13511278d321a908bd2'
                        examples.append(
                            InputExample(
                                example_id=e_id,
                                tokens=data['content'][mention['sent_id']]['tokens'],
                                triggerL=mention['offset'][0],  #触发词边界
...
def convert_examples_to_features(
    ...
    np.save('./data/my_file.npy', label_map)

    features = []
    for (ex_index, example) in tqdm.tqdm(enumerate(examples), desc="convert examples to features"):
        if ex_index % 10000 == 0:
            logger.info("Writing example %d of %d" % (ex_index, len(examples)))
        trigger = " ".join(example.tokens[example.triggerL:example.triggerR])
        trigger_token = tokenizer.tokenize(trigger)
        text = ['[unused1]'] + tokenizer.tokenize(" ".join(example.tokens[:]))
        # text = ['[unused1]'] + tokenizer.tokenize(" ".join(example.tokens[:example.triggerL])) + ['[unused1]'] + tokenizer.tokenize(" ".join(example.tokens[example.triggerL:example.triggerR]))+['[unused1]']+tokenizer.tokenize(" ".join(example.tokens[example.triggerR:]))
        ...
        maskL = [0.0]+[1.0 for i in range(0,len(trigger_token))] + [0.0 for i in range(0,len(text)+1)]
        maskR = [0.0 for i in range(0,len(trigger_token)+1)] + [1.0 for i in range(0,len(text))]+[0.0]
        ...
        inputs = tokenizer.encode_plus(
            trigger_token + text, add_special_tokens=True, max_length=max_length, return_token_type_ids=True
        )
