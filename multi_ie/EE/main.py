...
def build_dataset(fold:int, reader:Reader, train_max_len:int, dev_max_len:int, tokenizer, train_batch_size:int, dev_batch_size:int, rel2id:dict):
    ...
    train_data = []
    dev_data = []
    data_path = config.data_out_dir #os.path.join(config.data_out_dir, config.common["exp_name"])
    ...
    ''' 
    with open(os.path.join(data_path, 'fixed_train.json'), 'r') as f:
        train_data.extend(json.load(f))

    with open(os.path.join(data_path, 'fixed_dev.json'), 'r') as f:
        dev_data.extend(json.load(f))
    ...
    train_dataloader = train_features.get_dataloader(train_batch_size, num_workers=0, shuffle=True)
    dev_dataloader = dev_features.get_dataloader(dev_batch_size, num_workers=0, shuffle=True)
...
def main():
    ...
    #with open(os.path.join(config.data_out_dir, config.common["exp_name"], 'tag2id.json'), 'r') as f:
        #rel2id = json.load(f)
    tag = []
    with open(os.path.join(config.data_out_dir, 'schema.csv'), 'r') as f:
            line_count = 0
            for l in f:
                if line_count == 0:
                    line_count += 1
