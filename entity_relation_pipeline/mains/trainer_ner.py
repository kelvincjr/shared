...
class Trainer:
    ...
    def train(self):
        ...
        print('self.num_sample_total: ', self.num_sample_total)
        ...
        for epoch in range(self.config.epochs):
            ...
            for i, data_item in pbar:
                ...
                loss_ner_total += loss_ner.item()
                f1_ner_total += f1_ner.item()
                
            #if (epoch+1) % 1 == 0:
                #self.predict_sample()
            loss_ner_train_ave = loss_ner_total/self.num_sample_total
            print("train ner loss: {0}, f1 score: {1}".format(loss_ner_total/len(self.train_dataset),
                                                        f1_ner_total/len(self.train_dataset)))
    ...
    def evaluate(self):
        ...
        eval_sample_total = len(self.dev_dataset) * self.config.batch_size
        print('len(self.train_dataset): ', len(self.dev_dataset))
        print('self.config.batch_size: ', self.config.batch_size)
        self.model.train(False)
        pbar_dev = tqdm(enumerate(self.dev_dataset), total=len(self.dev_dataset))
        
        loss_total, loss_ner_total, f1_ner_total = 0, 0, 0
        ...
        print("eval ner loss: {0}, f1 score: {1}".format(loss_ner_total/len(self.dev_dataset),
                                                        f1_ner_total/len(self.dev_dataset)))
