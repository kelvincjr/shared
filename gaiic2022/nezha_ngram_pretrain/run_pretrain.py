...
from util.others.hanzi import punctuation
from util.pretrain_utils.trainer import Trainer
from util.modeling.modeling_nezha.modeling import NeZhaConfig, NeZhaForMaskedLM
...
def read_data_jd(args, pretrain_file_path, tokenizer: BertTokenizer) -> dict:
    max_len = 0
    len_512_count = 0
    with open(pretrain_file_path + 'unlabeled_train_data.txt', 'r', encoding='utf-8') as f:
        training_texts = []
    ...
    if args.debug:
        training_texts = training_texts[:300]

    inputs = defaultdict(list)
    for text in tqdm(training_texts, desc='', total=len(training_texts)):
...
def main():
    ...
    data = read_data_jd(args, args.pretrain_data_path, tokenizer)
