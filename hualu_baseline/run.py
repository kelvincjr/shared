...
def evaluate(args, eval_iter, model, mode, id2predicate):
    logger.info("***** Running Evalation *****")

    id2spo = {'predicate':{}, 'subject_type': {}, 'object_type': {}}
    for id_, predicate in id2predicate.items():
        items = predicate.split("_")
        if len(items) == 3:
            real_predicate = items[1]
            ...
            id2spo['predicate'][id_+309] = real_predicate
            id2spo['subject_type'][id_+309] = subject_type
            id2spo['object_type'][id_+309] = object_type
    ...
    if mode == "eval":
        precision, recall, f1 = get_precision_recall_f1("emergency_data/train/fixed_dev.json",
                                                        predict_file_path)
...
def main():
    ...
    train_dataset = DuIEDataset(args,
                                json_path="emergency_data/train/fixed_train.json",
                                tokenizer=tokenizer)
    eval_dataset = DuIEDataset(args,
                               json_path="emergency_data/train/fixed_dev.json",
                               tokenizer=tokenizer)
    ...
    for epoch, _ in enumerate(range(int(args.num_train_epochs))):
        ...
        # 每轮epoch在验证集上计算分数
        if epoch >= 0:
            eval_precision, eval_recall, eval_f1 = evaluate(args, eval_iter, model, mode="eval", id2predicate=id2predicate)
            print("precision: {:.4f}\n recall: {:.4f}\n f1: {:.4f}".format
