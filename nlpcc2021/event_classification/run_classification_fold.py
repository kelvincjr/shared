...
def train_and_eval(args, train_dataset, eval_dataset, model, tokenizer, labels, output_dir):
    """ Train the model """
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                        ...
                        if (
                            ...
                            results, _, _ = evaluate(args, eval_dataset, model, tokenizer, labels)
...
def evaluate(args, eval_dataset, model, tokenizer, labels, prefix=""):
...
def predict(args, test_dataset, model, tokenizer, labels, pad_token_label_id, prefix=""):
    args.eval_batch_size = args.per_gpu_eval_batch_size * max(1, args.n_gpu)
    # Note that DistributedSampler samples randomly
    ...
    logits = logits.tolist()        # 为了可以输出到json文件，需要转成list
    return [], batch_preds_list, logits
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The input data dir. Should contain the training files for the task.",
    )
    ...
    all_test_data, _ = load_and_cache_fold_examples(args, tokenizer, labels, mode="test1")
    ...
    for fold in range(args.start_fold, args.end_fold+1):
        ...
        if args.do_train:
            # Training
            global_step, tr_loss = train_and_eval(args, train_dataset, eval_dataset, model, tokenizer, labels, fold_output_dir)
        ...
        if args.do_eval:
            ...
            model.to(args.device)
            result, predictions, logits = evaluate(args, eval_dataset, model, tokenizer, labels,
                                                   prefix=fold_output_dir)
            # Save results
        ...
        if args.do_predict:
            ...
            model = model_class.from_pretrained(fold_output_dir)
            model.to(args.device)
            _, predictions, logits = predict(args, all_test_data, model, tokenizer, labels, pad_token_label_id)
            # Save results
