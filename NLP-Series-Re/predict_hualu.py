...
from run_mhs_hualu import evaluate
...
def main():
    ...
    with open('hualu_data/schema.csv') as f:
        predicate2type = {}
        line_count = 0
        for l in f:
            if line_count == 0:
    ...
    train_dataset = mhs_DuIEDataset(args,
                                    examples=read_examples(args, json_file="./hualu_data/fixed_train.json"),
                                    data_type="train")
    eval_dataset = mhs_DuIEDataset(args,
                                   examples=read_examples(args, json_file="./hualu_data/fixed_dev.json"),
                                   data_type="dev")
    ...
    res_dev = evaluate(args, eval_iter, model, mode="dev")
